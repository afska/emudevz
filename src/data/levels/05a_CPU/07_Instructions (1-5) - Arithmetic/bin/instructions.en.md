...
| `CMP` | Compare              | 🔢  | Compares `[A]` with a `value`, updating `Z`, `N` and `Z` as follows:<br><br>`Z` = `[A]` == `value`<br>`N` = (whether bit `7` of (`[A]` - `value`) is set)<br>`C` = `[A]` >= `value`                                                                                                           |
| `CPX` | Compare X Register   | 🔢  | Compares `[X]` with a `value`, updating `Z`, `N` and `Z` as follows:<br><br>`Z` = `[X]` == `value`<br>`N` = (whether bit `7` of (`[X]` - `value`) is set)<br>`C` = `[X]` >= `value`                                                                                                           |
| `CPY` | Compare Y Register   | 🔢  | Compares `[Y]` with a `value`, updating `Z`, `N` and `Z` as follows:<br><br>`Z` = `[Y]` == `value`<br>`N` = (whether bit `7` of (`[Y]` - `value`) is set)<br>`C` = `[Y]` >= `value`                                                                                                           |
