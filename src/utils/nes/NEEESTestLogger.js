...
const read = (cpu, address) => {
	if (cpu == null) throw new Error("`cpu` is null?!");
	if (cpu.memory == null) throw new Error("`cpu.memory` is null?!");
	if (typeof cpu.memory.read !== "function")
		throw new Error("`cpu.memory.read` is not a function?!");
...
export const getIndirectAddress = (cpu, address) => {
	...
	const low = read(cpu, start);
	const high = read(cpu, end);
...
export default class NEEESTestLogger {
	constructor() {
		this.lastLog = null;
	}
	...
	log(cpu, originalPC, operation, input, argument) {
		try {
			if (input != null && argument == null)
				throw new Error(`one of the addressing modes returned ${argument}?!`);
				...
				switch (operation.addressingMode.id) {
					...
					case "ABSOLUTE":
						...
						if (
							...
							$address += ` = ${hex(read(cpu, finalAddress), 2)}`;
						return $address;
					case "ZERO_PAGE":
						return `$${$input} = ${hex(read(cpu, finalAddress), 2)}`;
					case "INDEXED_ABSOLUTE_X":
					case "INDEXED_ZERO_PAGE_X":
						return `$${$input},X @ ${hexArgument(finalAddress)} = ${hex(
							read(cpu, finalAddress),
							2
						)}`;
					...
					case "INDEXED_INDIRECT":
						return `($${$input},X) @ ${hex(
							...
							2
						)} = ${hex(finalAddress, 4)} = ${hex(read(cpu, finalAddress), 2)}`;
			...
			if (operation.addressingMode.inputSize > 0) {
				$arguments +=
					operation.addressingMode.inputSize === 2
						? hex(byte.lowByteOf(input), 2) +
						  " " +
						  hex(byte.highByteOf(input), 2)
						: hex(input, 2);
			}
